...
def is_valid_sparql_local_name(value: str) -> bool:
    """
    Return True if `value` can be used as the local part of a SPARQL prefixed
    name (i.e. ns_prefix:value is syntactically valid).

    SPARQL/Turtle local names must not contain '/', '(', ')', '.', ',', ';',
    ':', '#', '@', '~', '!' or whitespace.

    YAGO-style predicates ("wasBornIn") → safe.
    FB15K-237-style paths ("/people/person/nationality") → unsafe.
    """
    if not value:
        return False
    INVALID_CHARS = set('/().,;:#@~! \t\n')
...
def build_sparql_query(head_parsed, body_parsed,
    ...
    def term(t: str, is_predicate: bool = False) -> str:
        ...
        Format one SPARQL term:
          ...
          - Already a URI (<..>) → returned as-is
          - Safe local name      → ns_prefix:value   (e.g. yago:wasBornIn)
          - Unsafe path          → <namespace/value>  (e.g. <http://freebase.org/people/...>)
        """
        if t.startswith('?'):
            return t
    ...
    # Emit PREFIX declaration only when prefixed notation will actually be used
    uses_prefix = (
        any(is_valid_sparql_local_name(bp) for _, bp, _ in body_parsed)
    ...
    head_line = (
        f'    {term(head_var)} {term(head_pred, is_predicate=True)} {term(head_obj)} .'
    )

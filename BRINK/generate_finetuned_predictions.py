"""Generate BRINK predictions with the TSV rule prepended to each question."""
...
def parse_list_cell(value: str, row_number: int, column: str) -> list[str]:
    """Parse a Python-list TSV cell, for example ``['rule one']``."""
    try:
        parsed = ast.literal_eval(value)
    except (SyntaxError, ValueError) as exc:
        raise ValueError(
            f"Row {row_number}: invalid {column} value {value!r}"
        ) from exc
...
def load_contexts(path: Path) -> dict[str, dict[str, list[str]]]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as file:
        ...
        if missing:
            raise ValueError(
                "Question TSV is missing column(s): " + ", ".join(sorted(missing))
            )
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            contexts[question_id] = {
                "rules": parse_list_cell(row["rule"] or "", row_number, "rule"),
            }
    return contexts


def build_user_prompt(question: str, context: dict[str, list[str]]) -> str:
    rules = " ".join(
        f"{rule.rstrip('.')}." for rule in context["rules"]
    )
    return f"{PROMPT_INSTRUCTION} {rules} {question}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate fine-tuned qKG predictions with rule context."
    )
...
def main() -> None:
    ...
    contexts = load_contexts(args.questions)
    ...
    missing_contexts = [
        str(item["id"]) for item in pending if str(item["id"]) not in contexts
    ]
    if missing_contexts:
        raise ValueError(
            "No rule context for ID(s): " + ", ".join(missing_contexts)
        )
    if not pending:
    ...
    for index, item in enumerate(pending, start=1):
        ...
        messages = [
            {
                "role": "user",
                "content": build_user_prompt(item["question"], contexts[question_id]),
            }
        ]
